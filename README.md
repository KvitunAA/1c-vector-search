...
**БД на другом сервере?** `sqlite-vec` и `Kuzu` — встраиваемые движки без сетевого протокола,
подключиться к ним «по IP» нельзя. Вместо этого MCP-сервер запускается на той машине, где лежат
файлы БД, а его stdio пробрасывается по SSH: см. [SSH_REMOTE.md](SSH_REMOTE.md) и
[`run_server_ssh.cmd`](run_server_ssh.cmd).

**Подключить команду?** Одна общая непривилегированная учётка на сервере, свой ключ у каждого,
конфиги для Cursor и Codex — см. [SCALING.md](SCALING.md) и [`add_mcp_user_key.ps1`](add_mcp_user_key.ps1).
Проверено: 4 одновременные сессии, ~250 МБ на каждую.

**Доступ из ChatGPT в браузере?** Через Secure MCP Tunnel: сервер остаётся в локальной сети,
наружу идёт только исходящее соединение — см. [CHATGPT_CONNECTOR.md](CHATGPT_CONNECTOR.md) и
[`server_http.py`](server_http.py) (транспорт Streamable HTTP поверх тех же инструментов).
